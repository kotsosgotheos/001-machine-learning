...
class KMeans:
    ...
    def __init__(self, n_clusters=10, max_iter=500): # defining the parameters 
        self.n_clusters = n_clusters
        self.max_iter = max_iter
        self.loss_per_iteration = []

    def init_centroids(self):
        np.random.seed(np.random.randint(0, 100000)) # adding the random seed
        self.centroids = []
    ...
    def calculate_accuracy(self):    #accuracy calculation
        self.clusters_labels = []
        self.clusters_info = []
        self.clusters_accuracy = []
        for clust, labels in list(self.clusters['labels'].items()): #adding the labels
